...
roomList.Count];
        ...
        {
            for (int i = 0; i < NumhealthOrbs; i++)
            {
                int RandomRoomIndex = Random.Range(1, roomList.Count);
                Room Room = roomList[RandomRoomIndex];
                Vector3Int SpawnPosition = Room.GetRandomPositionInRoom();
                SpawnPosition.y = 2;

                // hier checkt die of de Enemy niet op een van de andere objecteSpawnt
                while (SpawnPosition == PortalSpawnLocation || SpawnPosition == KeySpawnLocation || SpawnPosition == PlayerSpawnPosition || SpawnPosition == enemySpawnPosition)
                {
                    SpawnPosition = Room.GetRandomPositionInRoom();
                }

                Instantiate(HealthOrbs, SpawnPosition, Quaternion.identity);
            }
        }
